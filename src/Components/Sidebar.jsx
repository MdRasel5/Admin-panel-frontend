...
const Sidebar = ({ darkMode, toggleDarkMode }) => {
  ...
  return (
    <div className="w-full h-full bg-white dark:bg-dark-bg border-r-2 border-gray-600 dark:border-dark-border-color ">
      <div className="flex items-center justify-center text-gray-100 pt-6">
        ...
        />
        <h1
          className="text-2xl font-semibold ms-3 text-black dark:text-white"
          id="heading"
        >
          Admin Panel
        </h1>
        ...
        {ListItems.map((item, i) => (
          <Link
            ...
            className={`flex items-center  font-medium  text-gray-700 dark:text-gray-300 ${
              page_path === item.link
                ? "bg-slate-600/40 dark:bg-slate-400/40"
                : ""
      ...
      <div className="ldt">
        <label className="themeSwitcherTwo relative inline-flex cursor-pointer select-none items-center">
          ...
          <span
            className={`slider mx-[1vw] flex h-[2.5vw] w-[5vw] items-center rounded-full p-1 duration-200 ${
              darkMode ? "bg-[#FFFFFF]" : "bg-[#000000]"
            }`}
          >
            <span
              className={`dot h-[2vw] w-[2vw] rounded-full bg-white dark:bg-black  duration-200 ${
                darkMode ? "translate-x-[2vw]" : ""
              }`}
            ></span>
          </span>
          <span
